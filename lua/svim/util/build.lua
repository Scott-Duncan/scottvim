...
function M.keymaps()
	...
	group = "General"
	dofile(root .. "/lua/svim/config/keymaps.lua")
	group = "LSP"
	local lsp = dofile(root .. "/lua/svim/plugins/lsp/keymaps.lua")
	...
	local core = require("lazy.core.plugin").Spec.new({ import = "svim.plugins" })
	...
	Util.walk(root .. "/lua/svim/plugins/extras", function(path, name, t)
		if t == "file" and name:find("%.lua$") then
			local modname = path:gsub(".*/lua/", ""):gsub("/", "."):gsub("%.lua$", "")
			local extra_doc = "/plugins/extras/" .. modname:gsub("svim%.plugins%.extras%.", "")
...
```]]):format(
				name,
				Util.read_file(vim.fn.fnamemodify(root .. "/../svim-starter/lua/config/" .. name .. ".lua", ":p"))
			),
			"",
			"</TabItem>",
			([[<TabItem value="defaults" label="Default %s">]]):format(title),
			"",
			([[```lua title="svim.config.%s"
%s
```]]):format(name, Util.read_file(vim.fn.fnamemodify(root .. "/lua/svim/config/" .. name .. ".lua", ":p"))),
			"",
			"</TabItem>",
			"</Tabs>",
			"",
...
function M.recipes()
	local src = Util.read_file(vim.fs.normalize("~/projects/svim.github.io/lua/recipes.lua"))
...
function M.update2()
	local docs = vim.fs.normalize("~/projects/svim.github.io/docs")

	local config = Docs.extract("lua/svim/config/init.lua", "\nlocal defaults = ({.-\n})")
	...
	Docs.save({
		lazy = {
			content = [[```lua title="lua/config/lazy.lua"]] .. "\n" .. Util.read_file(
				vim.fn.fnamemodify(root .. "/../svim-starter/lua/config/lazy.lua", ":p")
			) .. "\n```",
		},
	...
	Util.walk(root .. "/lua/svim/plugins/extras", function(path, name, type)
...
```
	...
	local examples = vim.fn.fnamemodify(root .. "/../svim-starter/lua/plugins/example.lua", ":p")
	Docs.save({
...
function M.plugins(path)
	local test = root .. "/lua/svim/plugins/" .. path
